...
test("daemon dispatch captures, searches, and dismisses without binding a listener", async () => {
    ...
    try {
        ...
        assert.equal(captureRecord.item?.kind, "procedure");
        assert.equal(captureRecord.item?.retrieval, "context");
        ...
        assert.equal(contextRecord.project.length, 1);
        ...
        assert.equal((searchRecord.results[0] || {}).retrieval, "context");
        const itemId = captureRecord.item?.id || "";
        const dismiss = await daemon.dispatch({
            command: "dismiss",
            args: { id: itemId }
        });
        const dismissRecord = dismiss;
        assert.equal(dismissRecord.item?.status, "dismissed");
    }
    finally {
