...
import { pushBackup } from "../src/backup";
...
function makeConfig(tempRoot: string, remoteRepo: string) {
...
test("pushBackup exports canonical memory and projections to a git remote", async () => {
  ...
  try {
    ...
    assert.equal((capture as Record<string, unknown>).ok, true);
    ...
    const exported = JSON.parse(memoryJson) as {
